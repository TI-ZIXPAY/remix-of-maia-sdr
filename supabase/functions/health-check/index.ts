...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (settingsError) {
      ...
      let whatsappDetails: Record<string, unknown> = {};
      
      if (isUazapi) {
        // Only check active instances in uazapi_instances table
        const { count: instanceCount } = await supabase
          .from('uazapi_instances')
          .select('*', { count: 'exact', head: true })
          .eq('is_active', true);

        const hasActiveInstances = (instanceCount || 0) > 0;

        // Only consider active instances — ignore legacy nina_settings fields
        whatsappConfigured = hasActiveInstances;
        whatsappDetails = {
          provider: 'uazapi',
      ...
      if (whatsappConfigured) {
        results.push({
          component: 'whatsapp',
          status: 'ok',
          message: isUazapi ? `Uazapi configurado (${whatsappDetails.activeInstances} instância(s) ativa(s))` : 'WhatsApp Cloud configurado',
        ...
        results.push({
          component: 'whatsapp',
          status: 'warning',
          message: isUazapi ? 'Nenhuma instância Uazapi ativa' : 'WhatsApp não está totalmente configurado',
          details: whatsappDetails,
        });
      }
      ...
      if (settings.company_name && settings.sdr_name) {
        results.push({
          ...
          details: { companyName: settings.company_name, sdrName: settings.sdr_name },
        });
      } else {
        results.push({
          component: 'identity',
          ...
          details: { hasCompanyName: !!settings.company_name, hasSdrName: !!settings.sdr_name },
        });
      }
      ...
      const { data: bhSchedule } = await supabase
        .from('business_hours_schedule')
        .select('*')
        .eq('is_active', true);

      if (bhSchedule && bhSchedule.length > 0) {
        results.push({
          component: 'business_hours',
          status: 'ok',
          message: `Horário comercial configurado (${bhSchedule.length} dias)`,
          details: { timezone: settings.timezone, activeDays: bhSchedule.length },
        });
      } else {
        ...
        if (businessHoursConfigured) {
          results.push({
            component: 'business_hours',
            status: 'ok',
            message: 'Horário comercial configurado (legado)',
            details: { timezone: settings.timezone, start: settings.business_hours_start, end: settings.business_hours_end },
          });
        } else {
          results.push({
            component: 'business_hours',
    ...
    if (stagesError) {
      results.push({ component: 'pipeline_stages', status: 'error', message: 'Erro ao verificar pipeline: ' + stagesError.message });
    } else if (!stages || stages.length === 0) {
      results.push({ component: 'pipeline_stages', status: 'error', message: 'Nenhum estágio de pipeline encontrado. Clique em "Inicializar Sistema".' });
    } else {
      results.push({ component: 'pipeline_stages', status: 'ok', message: `${stages.length} estágios de pipeline configurados`, details: { count: stages.length, stages: stages.map(s => s.title) } });
    }

    // 4. Check tag_definitions
    ...
    if (tagsError) {
      results.push({ component: 'tag_definitions', status: 'error', message: 'Erro ao verificar tags: ' + tagsError.message });
    } else if (!tags || tags.length === 0) {
      results.push({ component: 'tag_definitions', status: 'warning', message: 'Nenhuma tag configurada' });
    } else {
      results.push({ component: 'tag_definitions', status: 'ok', message: `${tags.length} tags configuradas`, details: { count: tags.length } });
    }
    ...
    if (teamsError) {
      results.push({ component: 'teams', status: 'error', message: 'Erro ao verificar equipes: ' + teamsError.message });
    } else if (!teams || teams.length === 0) {
      results.push({ component: 'teams', status: 'warning', message: 'Nenhuma equipe configurada' });
    } else {
      results.push({ component: 'teams', status: 'ok', message: `${teams.length} equipes configuradas`, details: { count: teams.length, teams: teams.map(t => t.name) } });
    }
