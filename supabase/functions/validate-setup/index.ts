...
serve(async (req) => {
  ...
  try {
    ...
    if (settingsError || !settings) {
      ...
      if (isUazapi) {
        // Only check active instances — ignore legacy nina_settings fields
        ...
        if (instanceCount && instanceCount > 0) {
          results.push({
            component: 'whatsapp',
            status: 'ok',
            message: `Uazapi configurado: ${instanceCount} instância(s) ativa(s)`,
          });
        } else {
          results.push({
            component: 'whatsapp',
            status: 'error',
            message: 'Nenhuma instância Uazapi ativa',
            details: 'Adicione e ative uma instância nas configurações de API',
          });
        }
      } else {
        // Validate WhatsApp Cloud API
      ...
      // Check ElevenLabs (optional) - just verify key exists
      if (settings.elevenlabs_api_key && settings.audio_response_enabled) {
        results.push({
        ...
        results.push({
          component: 'elevenlabs',
          status: 'ok',
          message: 'ElevenLabs configurado (áudio desativado)',
        });
      } else {
        results.push({
          component: 'elevenlabs',
      ...
      // Check Business Hours from schedule table
      ...
      if (bhSchedule && bhSchedule.length > 0) {
        results.push({
          ...
          status: 'ok',
          message: `Horário configurado (${bhSchedule.length} dias ativos)`,
        });
      } else if (settings.business_hours_start && settings.business_hours_end) {
        results.push({
          component: 'business_hours',
          status: 'ok',
