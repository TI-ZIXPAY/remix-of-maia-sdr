...
serve(async (req) => {
  ...
  try {
    ...
    // Normalize phone to canonical Brazilian format
    const canonicalPhone = normalizeBrazilianPhone(phone);
    const phoneVariants = getBrazilianPhoneVariants(phone);

    // Get or create contact (single-tenant - no user_id filter, with 9th digit fallback)
