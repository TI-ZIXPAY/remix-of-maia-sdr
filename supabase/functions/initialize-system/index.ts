...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (existingGlobalSettings) {
      ...
      // Create default business_hours_schedule
      console.log('[initialize-system] Creating business_hours_schedule...');
      ...
      const { error: scheduleError } = await supabase
        ...
        .upsert(defaultSchedule, { onConflict: 'day_of_week' });

      if (scheduleError) {
        console.error('[initialize-system] Error creating business_hours_schedule:', scheduleError);
      } else {
        console.log('[initialize-system] business_hours_schedule created');
      }
    }
