...
export const OnboardingBanner: React.FC<OnboardingBannerProps> = ({ onOpenWizard }) => {
  const { loading, isComplete, steps, completionPercentage, hasSeenWizard } = useOnboardingStatus();

  // Hide banner if loading, complete, wizard was seen, or all required steps are done
  const requiredSteps = steps.filter(s => s.isRequired);
  const allRequiredComplete = requiredSteps.every(s => s.isComplete);
  
  if (loading || isComplete || hasSeenWizard || allRequiredComplete) return null;
