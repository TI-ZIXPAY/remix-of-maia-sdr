...
import { Pencil, Trash2, GripVertical, Plus, Lock, Bot, User, AlertTriangle, Webhook, Copy } from 'lucide-react';
...
export function PipelineSettingsModal({ open, onClose, onSave }: PipelineSettingsModalProps) {
  ...
  const [editWebhookEndpointId, setEditWebhookEndpointId] = useState<string>('');
  const [newStageWebhookEndpointId, setNewStageWebhookEndpointId] = useState<string>('');
  ...
  const handleEdit = (stage: KanbanColumn) => {
    ...
    setEditWebhookEndpointId(stage.webhookEndpointId || '');
  };
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await api.updatePipelineStage(editingId, {
        ...
        webhookEndpointId: editWebhookEndpointId && editWebhookEndpointId !== 'none' ? editWebhookEndpointId : null,
      } as any);
  ...
  const handleAddStage = async () => {
    ...
    try {
      await api.createPipelineStage({
        ...
        webhookEndpointId: newStageWebhookEndpointId && newStageWebhookEndpointId !== 'none' ? newStageWebhookEndpointId : null,
      });
              ...
              {loading ? (
                ...
                stages.map((stage, index) => (
                  <div
                    ...
                    className={`group flex items-center gap-3 p-3 border rounded-lg bg-card cursor-move hover:bg-accent/50 transition-colors ${
                  ...
                  >
                    ...
                    {editingId === stage.id ? (
                      <div className="flex-1 space-y-2">
                        ...
                        {/* Webhook Endpoint Selector */}
                        {webhookEndpoints.length > 0 && (
                          <div>
                            <Label className="text-xs text-muted-foreground flex items-center gap-1">
                              <Webhook className="w-3 h-3" />
                              Webhook ao entrar nesta etapa:
                            </Label>
                            <Select value={editWebhookEndpointId} onValueChange={setEditWebhookEndpointId}>
                      ...
                      <>
                        <div className="flex items-center gap-2 flex-1">
                          ...
                          {stage.webhookEndpointId && (
                            <div title={`Webhook: ${webhookEndpoints.find(e => e.id === stage.webhookEndpointId)?.name || 'Configurado'}`}>
            ...
            <div className="border-t pt-4 space-y-3">
              ...
              {webhookEndpoints.length > 0 && (
                <div>
                  ...
                  <Select value={newStageWebhookEndpointId} onValueChange={setNewStageWebhookEndpointId}>
                    <SelectTrigger className="h-8 text-sm">
                      <SelectValue placeholder="Nenhum" />
                    </SelectTrigger>
                    <SelectContent>
