...
export const MobileSidebar = ({
  ...
  return (
    <>
      <div
        className={cn(
          "h-14 px-4 flex flex-row md:hidden items-center justify-between bg-background border-b border-border/50 w-full flex-shrink-0 z-40"
        )}
        {...props}
      >
        <div className="flex justify-end z-20 w-full">
          <Menu
            className="text-foreground cursor-pointer hover:text-primary transition-colors w-6 h-6"
            onClick={() => setOpen(!open)}
          />
        </div>
        <AnimatePresence>
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        isActive
          ? "bg-secondary text-primary shadow-lg shadow-black/20 ring-1 ring-border"
    ...
    >
      {isActive && (
        <div className="absolute left-0 top-0 bottom-0 w-1 bg-primary rounded-l-md shadow-[0_0_10px_hsl(217_91%_55%/0.5)]" />
      )}
      <span className={cn(
