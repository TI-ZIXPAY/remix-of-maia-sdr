...
const Team: React.FC = () => {
  ...
  const [recentLeads, setRecentLeads] = useState<Record<string, number>>({});
  ...
  const loadAllData = async () => {
    ...
    try {
      ...
      // Fetch recent lead counts from roulette_assignments (last 7 days)
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowModal(false);
      setFormData({ name: '', email: '', role: 'agent', team_id: '', function_id: '', weight: 1, external_id: '' });
      await loadAllData();
    } catch (error) {
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.updateTeamMember(editingMember.id, {
        ...
        external_id: editFormData.external_id || null
      });
      toast.success('Membro atualizado com sucesso!');
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="border-b border-slate-800/50">
                            ...
                            <th className="px-6 py-4 text-xs font-medium text-slate-500 uppercase tracking-wider">Leads (7d)</th>
                    ...
                    <tbody className="divide-y divide-slate-800/30">
                        {filteredMembers.map((member) => (
                            <tr key={member.id} className="hover:bg-slate-800/20 transition-colors group">
                                ...
                                {/* Recent Leads */}
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <span className="inline-flex items-center justify-center min-w-[2rem] px-2 py-1 rounded-full text-xs font-bold bg-cyan-500/10 text-cyan-400 border border-cyan-500/20">
                                        {recentLeads[member.id] || 0}
                                    </span>
                                </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-slate-900 border border-slate-800 rounded-xl shadow-2xl max-w-md w-full overflow-hidden animate-in zoom-in-95 duration-200">
                ...
                <form onSubmit={handleInvite} className="p-6 space-y-4">
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-slate-300">ID CRM Externo (opcional)</label>
                        <input
                            type="text"
                            ...
                            className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2.5 text-sm text-white focus:ring-1 focus:ring-slate-600 outline-none transition-all"
                        />
                        <p className="text-xs text-slate-500">ID do usuário no CRM externo para mapeamento na roleta</p>
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-slate-900 border border-slate-800 rounded-xl shadow-2xl max-w-md w-full overflow-hidden animate-in zoom-in-95 duration-200">
                ...
                <form onSubmit={handleEditSubmit} className="p-6 space-y-4">
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-slate-300">ID CRM Externo</label>
                        <input
                            type="text"
                            placeholder="Ex: 12345"
                            ...
                            className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2.5 text-sm text-white focus:ring-1 focus:ring-slate-600 outline-none transition-all"
                        />
                        <p className="text-xs text-slate-500">ID do usuário no CRM externo</p>
                    </div>
