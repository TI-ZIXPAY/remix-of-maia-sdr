...
import { X, ChevronLeft, ChevronRight, Loader2, Settings2, SkipForward, Ban } from 'lucide-react';
...
export const OnboardingWizard: React.FC<OnboardingWizardProps> = ({ isOpen, onClose }) => {
  ...
  // Form state - WhatsApp (provider selection)
  const [whatsappProvider, setWhatsappProvider] = useState<'cloud' | 'uazapi'>('cloud');
  // Form state - WhatsApp Cloud API
  const [accessToken, setAccessToken] = useState('');
  const [phoneNumberId, setPhoneNumberId] = useState('');
  const [businessAccountId, setBusinessAccountId] = useState('');
  const [verifyToken, setVerifyToken] = useState('');
  // Form state - Uazapi
  const [uazapiEndpoint, setUazapiEndpoint] = useState('');
  const [uazapiSession, setUazapiSession] = useState('');
  const [uazapiSessionkey, setUazapiSessionkey] = useState('');
  ...
  // Load user role (admin vs user) to avoid RLS failures when saving
  useEffect(() => {
    const loadRole = async () => {
      ...
      try {
        ...
        if (error) {
          console.error('[OnboardingWizard] Error loading user role:', error);
          setIsAdmin(false);
          return;
        }
        ...
        console.error('[OnboardingWizard] Unexpected error loading user role:', e);
        setIsAdmin(false);
      } finally {
    ...
    const initializeAndLoad = async () => {
      ...
      try {
        ...
        if (data) {
          ...
          setBrokerName((data as any).broker_name || '');
          setBrokerPhone((data as any).broker_phone || '');
          
          // WhatsApp provider
          setWhatsappProvider(((data as any).whatsapp_provider as 'cloud' | 'uazapi') || 'cloud');
          // WhatsApp Cloud API
          ...
          // Uazapi
          setUazapiEndpoint((data as any).uazapi_endpoint || '');
          setUazapiSession((data as any).uazapi_session || '');
          setUazapiSessionkey((data as any).uazapi_sessionkey || '');
  ...
  const saveSettings = useCallback(async () => {
    ...
    if (!isAdmin) {
      console.warn('[OnboardingWizard] ❌ User is not admin - blocking settings update');
      toast.error('Somente administradores podem salvar configurações.');
      return false;
    }
    ...
    try {
      ...
      const settings = {
        ...
        // WhatsApp provider
        whatsapp_provider: whatsappProvider || 'cloud',
        // WhatsApp Cloud API - trim whitespace
        ...
        // Uazapi
        uazapi_endpoint: uazapiEndpoint?.trim() || null,
        uazapi_session: uazapiSession?.trim() || null,
        uazapi_sessionkey: uazapiSessionkey?.trim() || null,
    ...
    timezone, businessHoursStart, businessHoursEnd, businessDays, refetch, isAdmin
  ]);
      ...
      case 0:
        return (
          <StepIdentity
            ...
            onBrokerPhoneChange={setBrokerPhone}
          />
        );
      case 1:
        return (
          <StepWhatsApp
            ...
            uazapiEndpoint={uazapiEndpoint}
            uazapiSession={uazapiSession}
            uazapiSessionkey={uazapiSessionkey}
            onUazapiEndpointChange={setUazapiEndpoint}
            onUazapiSessionChange={setUazapiSession}
            onUazapiSessionkeyChange={setUazapiSessionkey}
            uazapiWebhookUrl={uazapiWebhookUrl}
          />
        );
      case 2:
        return (
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-6 custom-scrollbar overflow-x-hidden">
                {!isInitializing && !isRoleLoading && !isAdmin && (
                  <div className="mb-4 rounded-xl border border-amber-500/30 bg-amber-500/10 p-3">
                    <p className="text-sm text-amber-200">
                      Você está logado como <span className="font-semibold">usuário</span>. Apenas{' '}
                      <span className="font-semibold">administradores</span> podem salvar configurações do sistema.
                      Faça login com a conta admin (primeiro usuário) ou peça para um admin ajustar as credenciais.
                    </p>
                  </div>
                )}
                {isInitializing ? (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      <Button
                        variant="ghost"
                        onClick={() => {
                           if (window.confirm('Tem certeza que deseja cancelar?\n\nVocê poderá retomar a configuração depois em Configurações.')) {
                            onClose();
                          }
                        }}
                        className="gap-2 text-red-400/70 hover:text-red-400 hover:bg-red-500/10"
                      >
