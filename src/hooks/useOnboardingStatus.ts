      ...
      if (settings) {
        setSteps(prev => prev.map(step => {
          switch (step.id) {
            ...
            case 'whatsapp':
              // Check provider and validate corresponding fields
              const isUazapi = settings.whatsapp_provider === 'uazapi';
              const whatsappComplete = isUazapi
                ? !!(settings.uazapi_endpoint && settings.uazapi_session && settings.uazapi_sessionkey)
                : !!(settings.whatsapp_access_token && settings.whatsapp_phone_number_id && (settings as any).whatsapp_business_account_id);
              return {
                ...step,
            ...
            case 'verification':
              // Check verification based on provider
              const verifyWhatsapp = settings.whatsapp_provider === 'uazapi'
                ? !!(settings.uazapi_endpoint && settings.uazapi_session && settings.uazapi_sessionkey)
                : !!settings.whatsapp_access_token;
              return {
                ...step,
                isComplete: !!(settings.company_name && settings.sdr_name && verifyWhatsapp && settings.system_prompt_override),
              };
            case 'finish':
              return {
                ...step,
